...
public sealed class ForgeSearch<T>
{
    ...
    public ForgeSearch<T> Select(params string[] columns) { _columns.AddRange(columns); return this; }
    public ForgeSearch<T> From(string table) { _table = table; return this; }
    public ForgeSearch<T> FromSql(string sql) { _fromSql = sql; return this; }
    ...
    public ForgeSearch<T> WhereIf(bool condition, Expression<Func<T, bool>> predicate) => condition ? Where(predicate) : this;
    public ForgeSearch<T> Where(string sql, object? parameters = null) { _where.Add(sql); Merge(parameters); return this; }
    public ForgeSearch<T> WhereIf(bool condition, string sql, object? parameters = null) => condition ? Where(sql, parameters) : this;
    ...
    public ForgeSearch<T> OrderBy(string orderBy) { _orderBy = orderBy; return this; }
    public ForgeSearch<T> Page(int page, int pageSize) { _page = Math.Max(page, 1); _pageSize = Math.Max(pageSize, 1); return this; }
    ...
    public async Task<ForgePagedResult<T>> ToPagedAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new ForgePagedResult<T> { Items = items, Page = _page ?? 1, PageSize = _pageSize ?? items.Count, TotalRecords = total };
    }
...
public sealed class ForgeProcedureSearch<T>
{
    ...
    private int? _page;
    private int? _pageSize;
    public ForgeProcedureSearch(IForgeDb db, string procedureName) { _db = db; _procedureName = procedureName; }
    public ForgeProcedureSearch<T> With(string name, object? value) { _parameters[Normalize(name)] = value; return this; }
    public ForgeProcedureSearch<T> WithOptional(string name, object? value) { if (value is not null) With(name, value); return this; }
    ...
    public Task<IReadOnlyList<T>> ToListAsync(CancellationToken cancellationToken = default) => _db.QueryProcedureAsync<T>(_procedureName, _parameters, cancellationToken: cancellationToken);
    public async Task<ForgePagedResult<T>> ToPagedAsync(CancellationToken cancellationToken = default)
    {
        var items = await ToListAsync(cancellationToken);
        return new ForgePagedResult<T> { Items = items, Page = _page ?? 1, PageSize = _pageSize ?? items.Count, TotalRecords = items.Count };
    }
...
internal static class ForgeSearchExpression
{
    ...
    private static string Member(Expression expression) => expression is MemberExpression member ? member.Member.Name : throw new NotSupportedException("Left side must be a member expression.");
    private static string Operator(ExpressionType type) => type switch
    {
        ExpressionType.Equal => "=", ExpressionType.NotEqual => "<>", ExpressionType.GreaterThan => ">", ExpressionType.GreaterThanOrEqual => ">=", ExpressionType.LessThan => "<", ExpressionType.LessThanOrEqual => "<=", _ => throw new NotSupportedException($"Operator {type} is not supported.")
    };
    ...
    private static object? Evaluate(Expression expression) => Expression.Lambda(expression).Compile().DynamicInvoke();
}
