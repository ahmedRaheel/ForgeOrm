...
namespace ForgeORM.Providers.Sqlite.Graph;

/// <summary>
/// SQLite graph executor that routes graph nodes to transaction-based prepared commands and multi-row inserts.
/// </summary>
public sealed class SqliteForgeGraphExecutor : IForgeGraphExecutor
{
    ...
    public SqliteForgeGraphExecutor(IForgeGraphStrategySelector selector, IForgeForeignKeyBinder foreignKeyBinder)
    {
        _selector = selector;
        _foreignKeyBinder = foreignKeyBinder;
    }

    public ForgeDatabaseProvider Provider => ForgeDatabaseProvider.Sqlite;
    ...
    {
        // SQLite hook: transaction + prepared commands + multi-row insert where safe.
