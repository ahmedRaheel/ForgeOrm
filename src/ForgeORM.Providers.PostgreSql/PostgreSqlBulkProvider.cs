...
public sealed class PostgreSqlBulkProvider : IForgeBulkProvider
{
    ...
    public async ValueTask<int> UpdateBulkAsync<T>(
        ...
        CancellationToken cancellationToken = default)
    {
        await PostgreSqlNativeBulk.BulkUpdateAsync(connection, tableName, rows, keyColumn, options ?? ForgeProviderBulkOptionsDefaults.Current, cancellationToken).ConfigureAwait(false);
        return rows.Count;
    }
    ...
    public async ValueTask<int> DeleteBulkAsync<TKey>(
        ...
        CancellationToken cancellationToken = default)
    {
        await PostgreSqlNativeBulk.BulkDeleteAsync(connection, tableName, keys, keyColumn, options ?? ForgeProviderBulkOptionsDefaults.Current, cancellationToken).ConfigureAwait(false);
        return keys.Count;
    }
