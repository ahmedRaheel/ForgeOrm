...
namespace ForgeORM.Providers.MySql.Graph;

/// <summary>
/// MySQL graph executor that routes graph nodes to multi-row INSERT, temp table, and ON DUPLICATE KEY UPDATE strategies.
/// </summary>
public sealed class MySqlForgeGraphExecutor : IForgeGraphExecutor
{
    ...
    public MySqlForgeGraphExecutor(IForgeGraphStrategySelector selector, IForgeForeignKeyBinder foreignKeyBinder)
    {
        _selector = selector;
        _foreignKeyBinder = foreignKeyBinder;
    }

    public ForgeDatabaseProvider Provider => ForgeDatabaseProvider.MySql;
    ...
    {
        // MySQL hook: multi-row INSERT, temporary table, ON DUPLICATE KEY UPDATE, DELETE missing children.
