...
public static class ForgePerformancePipeline
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!ForgeEnterpriseRuntime.IsEnabled &&
            ForgeDirectQueryExecutor.TryQueryAsync<T>(connection, sql, parameters, transaction, commandType, timeoutSeconds, cancellationToken, out var directQuery))
            return await directQuery.ConfigureAwait(false);


        var plan = ForgeCompiledExecutionPlanCache.GetOrAdd<T>(connection, sql, parameters, commandType, CommandBehavior.Default);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var enterpriseEnabled = ForgeEnterpriseRuntime.IsEnabled;
        ForgeCommandExecutionContext context = default;
        Stopwatch? stopwatch = null;
        try
        {
            if (enterpriseEnabled)
            {
                context = ForgeEnterpriseRuntime.CreateContext(command, ForgeCommandOperation.Stream, typeof(T), plan.ParameterType, plan.QueryFingerprint);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!ForgeEnterpriseRuntime.IsEnabled &&
            ForgeDirectQueryExecutor.TryFirstOrDefaultAsync<T>(connection, sql, parameters, transaction, commandType, timeoutSeconds, cancellationToken, out var directFirst))
            return await directFirst.ConfigureAwait(false);

        var plan = ForgeCompiledExecutionPlanCache.GetOrAdd<T>(connection, sql, parameters, commandType, CommandBehavior.SingleRow | CommandBehavior.Default);
        await using var command = CreateCommand(connection, plan, parameters, transaction, timeoutSeconds);
    ...
    {
        if (!ForgeEnterpriseRuntime.IsEnabled &&
            ForgeDirectQueryExecutor.TrySingleOrDefaultAsync<T>(connection, sql, parameters, transaction, commandType, timeoutSeconds, cancellationToken, out var directSingle))
    ...
    {
        if (!ForgeEnterpriseRuntime.IsEnabled &&
            ForgeDirectQueryExecutor.TryExecuteAsync(connection, sql, parameters, transaction, commandType, timeoutSeconds, cancellationToken, out var directExecute))
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!ForgeEnterpriseRuntime.IsEnabled &&
            ForgeDirectQueryExecutor.TryScalarAsync<T>(connection, sql, parameters, transaction, commandType, timeoutSeconds, cancellationToken, out var directScalar))
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (enterpriseEnabled)
            {
                context = ForgeEnterpriseRuntime.CreateContext(command, operation, typeof(T), plan.ParameterType, plan.QueryFingerprint);
                stopwatch = Stopwatch.StartNew();
                await ForgeEnterpriseRuntime.OnExecutingAsync(command, context, cancellationToken).ConfigureAwait(false);
            }
    ...
    private static async ValueTask<int> ExecuteNonQueryWithEnterpriseHooksAsync<T>(DbCommand command, ForgeCompiledQueryPlan<T> plan, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (enterpriseEnabled)
            {
                context = ForgeEnterpriseRuntime.CreateContext(command, ForgeCommandOperation.Execute, typeof(T), plan.ParameterType, plan.QueryFingerprint);
                stopwatch = Stopwatch.StartNew();
                await ForgeEnterpriseRuntime.OnExecutingAsync(command, context, cancellationToken).ConfigureAwait(false);
            }
    ...
    private static async ValueTask<object?> ExecuteScalarWithEnterpriseHooksAsync<T>(DbCommand command, ForgeCompiledQueryPlan<T> plan, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (enterpriseEnabled)
            {
                context = ForgeEnterpriseRuntime.CreateContext(command, ForgeCommandOperation.Scalar, typeof(T), plan.ParameterType, plan.QueryFingerprint);
                stopwatch = Stopwatch.StartNew();
                await ForgeEnterpriseRuntime.OnExecutingAsync(command, context, cancellationToken).ConfigureAwait(false);
            }
    ...
    private static DbCommand CreateCommand<T>(DbConnection connection, ForgeCompiledQueryPlan<T> plan, object? parameters, DbTransaction? transaction, int? timeoutSeconds)
    {
        ...
        if (plan.RequiresEnumNormalization)
        {
