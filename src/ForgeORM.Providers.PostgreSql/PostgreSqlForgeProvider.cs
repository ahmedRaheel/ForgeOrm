...
public sealed class PostgreSqlForgeProvider : IForgeDatabaseProvider
{
    ...
    public ForgeCommand BuildGetByIds(ForgeEntityMetadata e, IReadOnlyCollection<int> ids) { return ForgeCommand.Text($"SELECT {BuildProjection(e)} FROM {e.TableName} WHERE {e.KeyColumn} = ANY(@Ids)", new { Ids = ids.ToArray() }); }
    /// <summary>
