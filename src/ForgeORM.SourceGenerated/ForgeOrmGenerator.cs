...
public sealed class ForgeOrmGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(SourceProductionContext context, IEnumerable<INamedTypeSymbol> allTypes)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        if (!IsSqlServerFirstCandidate(sql, parameters))");
        sb.AppendLine("        {");
    ...
    private static void EmitHelpers(StringBuilder sb)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    private static bool IsSqlServerFirstCandidate(string sql, object? parameters)");
        sb.AppendLine("        => parameters is not null && !string.IsNullOrWhiteSpace(sql) && sql.IndexOf('@') >= 0 && sql.IndexOf(\" IN \", StringComparison.OrdinalIgnoreCase) < 0;");
        sb.AppendLine();
        sb.AppendLine("    private static string FirstSqlParameterName(string sql)");
        sb.AppendLine("    {");
        sb.AppendLine("        for (var i = 0; i < sql.Length; i++)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (sql[i] != '@') continue;");
        sb.AppendLine("            var start = i++;");
        sb.AppendLine("            while (i < sql.Length && (char.IsLetterOrDigit(sql[i]) || sql[i] == '_')) i++;");
        sb.AppendLine("            return sql.Substring(start, i - start);");
        sb.AppendLine("        }");
