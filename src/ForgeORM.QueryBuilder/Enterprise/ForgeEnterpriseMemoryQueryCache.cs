...
/// </summary>
public sealed class ForgeEnterpriseMemoryQueryCache : IForgeEnterpriseQueryCache
{
    ...
    public ValueTask SetAsync<T>(string key, T value, ForgeQueryCacheOptions options, CancellationToken cancellationToken = default)
    {
        _items[key] = new CacheEntry(value, DateTimeOffset.UtcNow.Add(options.Duration), options.Region);
        return ValueTask.CompletedTask;
    }

    public ValueTask RemoveRegionAsync(string region, CancellationToken cancellationToken = default)
    {
