...
public sealed class PostgreSqlBulkProvider : IForgeBulkProvider
{
    ...
    {
        await PostgreSqlNativeBulk.BulkInsertAsync(connection, tableName, rows, cancellationToken).ConfigureAwait(false);
        return rows.Count;
    }
    ...
    public async ValueTask<int> UpdateBulkAsync<T>(
        ...
        CancellationToken cancellationToken = default)
    {
        await PostgreSqlNativeBulk.BulkUpdateAsync(connection, tableName, rows, keyColumn, cancellationToken).ConfigureAwait(false);
        return rows.Count;
    }
